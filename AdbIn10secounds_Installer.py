...
def download_platform_tools():
    ...
    if response.status_code == 200:
        ...
        print("Failed to download platform-tools. Please check your internet connection.")
...
def main():
    ...
    while install_response not in ['0', '1']:
        install_response = input("Invalid input. Please enter 1 for Yes or 0 for No: ")
    ...
    if download_success:
        ...
        while path_response not in ['0', '1']:
            path_response = input("Invalid input. Please enter 1 for Yes or 0 for No: ")

        if path_response == '1':
            path_success = add_to_path(INSTALL_DIR)
            if path_success:
                print("Installation completed successfully! Press 1 to exit.")
            else:
                print("Installation failed while adding to PATH. Press 0 to exit.")
        else:
            print("Installation completed without adding to PATH. Press 1 to exit.")
    else:
        print("Installation failed. Please try again. Press 0 to exit.")
